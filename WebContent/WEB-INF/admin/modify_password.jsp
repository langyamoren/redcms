...
$(document).ready(function(){
	...
	$("#orgpwd").formValidator({onShow:"请输入密码",onFocus:"至少1个长度",onCorrect:"密码合法"}).inputValidator({min:1,empty:{leftEmpty:false,rightEmpty:false,emptyError:"密码两边不能有空符号"},onError:"密码不能为空,请确认"});
	$("#orgpwd").ajaxValidator(
			{
				type:"get",
				async:"true",
				buttons: $("#submm"),
				url:"admin/ajax/validatepwd",
				dataType:"text",
				data:"email=${sessionScope.manager.email}&id=${sessionScope.manager.id}",
	...
	$("#repwd").formValidator({onShow:"输再次输入密码",onFocus:"至少1个长度",onCorrect:"密码一致"}).inputValidator({min:1,empty:{leftEmpty:false,rightEmpty:false,emptyError:"重复密码两边不能有空符号"},onError:"重复密码不能为空,请确认"}).compareValidator({desID:"pwd",operateor:"=",onError:"2次密码不一致,请确认"});
});
</script>
</head>
<body>
<form action="admin/manager" method="post" id="form1">
...
<table width="500" border="0" align="center" cellpadding="0" cellspacing="0" class="redtable">
  ...
  <tr>
    ...
    <td width="140"><input name="orgpwd" type="password" class="redinput" id="orgpwd"></td>
    <td><div id="orgpwdTip"></div></td>
  </tr>
  <tr>
