...
@MultipartConfig
@WebServlet("/admin/upload_files")
public class UploadFilesServlet extends ServletBase
{
	private static final long serialVersionUID = -2992878748893555047L;
			...
			throws ServletException, IOException
	{
		...
		String realPath=this.getServletContext().getRealPath("res/attached");
		
		PartUitl pu=new PartUitl(part, "application/octet-stream,application/msword,application/zip",".zip,.rar,.doc", realPath, 1024*1024*10);
...
/*		 out.println("<h1>上传成功!</h1><ul>");
		  ...
		  out.println("</ul>");*/
		   ...
		   files.setPath("res/attached/"+pu.getNewName());
		   ...
		   int reid=0;
			...
			obj.put("error", 0);
			obj.put("url",req.getContextPath()+"/res/attached/"+pu.getNewName());
			obj.put("filesId", reid);
			obj.put("mime",pu.getContentType());
			obj.put("oldname", pu.getFileName());
